...
class SPNoise(object):
    ...
    def __call__(self,results):
        ...
        if random.random() < self.p:
            img1 = skimage.util.random_noise(img1, mode='s&p', var=0.001, clip=True)

            results['img'] = img1
            results['gt_bboxes'] = boxes1
            results['gt_labels'] = labels1
        else:pass
        return results
...
class GaussNoise(object):
    ...
    def __call__(self, results):
        ...
        if random.random() < self.p:
            img1 = skimage.util.random_noise(img1, mode='gaussian',var=0.0005, clip=True)#var?
            io.imshow(img1)
            plt.show()
            results['img'] = img1
            results['gt_bboxes'] = boxes1
...
@PIPELINES.register_module()
class BBoxJitter_ratio(object):
    def __init__(self, x_ratio=0.01, y_ratio=0.01, prob=0.5):
        # 1: return ori img
    ...
    def __call__(self, results):
        ...
        x_left_ratio = np.random.uniform(-self.x_ratio, self.x_ratio, size=(boxes.shape[0]))
        x_right_ratio = np.random.uniform(-self.x_ratio, self.x_ratio, size=(boxes.shape[0]))
        y_up_ratio = np.random.uniform(-self.y_ratio, self.y_ratio, size=(boxes.shape[0]))
        y_bottom_ratio = np.random.uniform(-self.y_ratio, self.y_ratio, size=(boxes.shape[0]))
        ...
        if np.random.rand() <= self.prob:
            boxes[:, 0] += x_left_ratio * w
            boxes[:, 1] += y_up_ratio * h
            boxes[:, 2] += x_right_ratio * w
            boxes[:, 3] += y_bottom_ratio * h

            boxes[:, 0::2] = np.clip(boxes[:, 0::2], 0, img.shape[1] - 1)
            boxes[:, 1::2] = np.clip(boxes[:, 1::2], 0, img.shape[0] - 1)
